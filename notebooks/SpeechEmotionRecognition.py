...
class speechEmotionRecognition:
    ...
    '''
    def __init__(self, subdir_model='../Model/MODEL_CNN_LSTM.hdf5'):

        # Load prediction model

        self._model = self.build_model()
        subdir_model = "../Model/MODEL_CNN_LSTM.hdf5"
    ...
    def voice_recording(self, filename, duration=5, sample_rate=16000, chunk=1024, channels=1):
        ...
        # Record audio until 'q' key is pressed
        stream.start_stream()
        while True:
    ...
    def voice_recording_multi(self, filename,exit_event, duration=5, sample_rate=16000, chunk=1024, channels=1):
    ...
    '''
    def prediction_to_csv(self, predictions,timestamps, filename, mode='w'):
        # Write emotion in filename
        with open(filename, mode) as f:
            if mode == 'w':
                f.write("TIMESTAMP, EMOTIONS"+'\n')
            i=0
            for emotion in predictions:
                f.write(str(timestamps[i])+", "+str(emotion)+'\n')
                i+=1
            f.close()
...
if __name__=="__main__":
    ...
    speech_recognizer.voice_recording(audio_filename, duration=20)
    ...
    # Export predicted emotions to a CSV file
    speech_recognizer.prediction_to_csv(emotions, "predicted_emotions.csv", mode='w')
